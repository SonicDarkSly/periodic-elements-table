  ...
  while (electronCount > 0) {
    ...
    for (var i = 0; i < electronsInLayer; i++) {
      ...
      var electron = createSphere({
        ...
        color: Colors.green,
        opacity: 0.675,
      }); // Couleur verte transparente pour les électrons
...
function createAtom(protonCount, neutronCount, electronCount) {
  ...
  var protonMaterial = new THREE.MeshPhongMaterial({
    ...
    transparent: true,
    opacity: 0.737,
  }); // Couleur rouge transparente pour les protons
  ...
  for (var i = 0; i < protonCount; i++) {
    ...
    proton.position.set(
      ...
      (Math.random() - 0.5) * 8,
      (Math.random() - 0.4) * 8
    ); // Augmentation de l'espace entre les protons
    nucleus.add(proton);
  }
  ...
  var neutronMaterial = new THREE.MeshPhongMaterial({
    ...
    transparent: true,
    opacity: 0.773,
  }); // Couleur bleue transparente pour les neutrons
  ...
  for (var i = 0; i < neutronCount; i++) {
    ...
    neutron.position.set(
      ...
      (Math.random() - 0.5) * 8,
      (Math.random() - 0.4) * 8
    ); // Augmentation de l'espace entre les neutrons
    nucleus.add(neutron);
  }
